...
outreg2 using ${output_dir}/PokeBallReg.doc, replace ctitle(DID Model)   keep(DID) dec(5) sdec(5) label addtext(City Fixed Effects, Yes, Year Fixed Effects, Yes) 
...
outreg2 using ${output_dir}/PokeBallReg.doc, append title(PokeBall Factories' effect on Asthma) ctitle(PokeBall Production Model)  addtext(City Fixed Effects, Yes, Year Fixed Effects, Yes)    keep(perCapitaProduction) dec(5) sdec(5) noni label
